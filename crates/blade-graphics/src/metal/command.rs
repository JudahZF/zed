...
impl super::RenderCommandEncoder<'_> {
    pub fn with<'p>(
        ...
        // Depth clip mode is not supported on iOS Simulator, so skip this call there.
        // The `target_abi = "sim"` check detects Apple Silicon simulators, while
        // `target_arch = "x86_64"` with `target_os = "ios"` detects Intel simulators.
        #[cfg(not(all(target_os = "ios", any(target_abi = "sim", target_arch = "x86_64"))))]
