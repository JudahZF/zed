...
impl PlatformAtlas for MetalAtlas {
    ...
    fn remove(&self, key: &AtlasKey) {
        let mut lock = self.0.lock();
        let Some(id) = lock.tiles_by_key.get(key).map(|value| value.texture_id) else {
            return;
        };
        ...
        let Some(texture_slot) = textures
            .textures
            .iter_mut()
            .find(|texture| texture.as_ref().is_some_and(|value| value.id == id))
        else {
            return;
        };
...
impl MetalAtlasState {
    ...
    fn push_texture(
        ...
        let atlas_texture = MetalAtlasTexture {
            ...
            allocator: BucketedAtlasAllocator::new(size_to_etagere(size)),
        ...
        if let Some(index) = index {
            ...
            texture_list.textures.get_mut(index)
        } else {
...
impl MetalAtlasTexture {
    ...
    fn bytes_per_pixel(&self) -> u8 {
        ...
        match self.metal_texture.pixel_format() {
            A8Unorm | R8Unorm => 1,
            RGBA8Unorm | BGRA8Unorm | RGBA8Unorm_sRGB | BGRA8Unorm_sRGB => 4,
            pixel_format => {
                log::error!(
                    "Unsupported iOS atlas texture format {:?}; assuming 4 bytes per pixel",
                    pixel_format
                );
                4
            }
