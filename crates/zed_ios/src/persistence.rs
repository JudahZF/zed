...
impl ConnectionDb {
    ...
    pub fn save_connection(
        ...
        conn.exec_bound::<(String, String, i64, Option<String>, i64, i64)>(
            "INSERT INTO saved_connections (hostname, username, port, nickname, created_at, last_used_at)
             VALUES (?, ?, ?, ?, ?, ?)
    ...
    pub fn recent_connections(&self, limit: usize) -> Result<Vec<SavedConnection>> {
        ...
        let connections = conn.select_bound::<i64, SavedConnection>(
            "SELECT id, hostname, username, port, nickname, created_at, last_used_at
    ...
    pub fn clear_all(&self) -> Result<()> {
        ...
        conn.exec("DELETE FROM saved_connections")?()
    ...
    pub fn count(&self) -> Result<usize> {
        ...
        let count = conn.select_row::<i64>(
            "SELECT COUNT(*) FROM saved_connections"
        )?()
...
mod tests {
    ...
    fn test_save_and_retrieve_connection() {
        ...
        db.save_connection("example.com", "user", 22, Some("My Server")).unwrap();
    ...
    fn test_upsert_updates_nickname() {
        ...
        db.save_connection("example.com", "user", 22, None).unwrap();
        db.save_connection("example.com", "user", 22, Some("Updated")).unwrap();
    ...
    fn test_find_connection() {
        ...
        db.save_connection("example.com", "user", 22, None).unwrap();
        db.save_connection("other.com", "admin", 2222, None).unwrap();
    ...
    fn test_recent_connections_limit() {
        ...
        db.save_connection("first.com", "user", 22, None).unwrap();
        db.save_connection("second.com", "user", 22, None).unwrap();
        db.save_connection("third.com", "user", 22, None).unwrap();
