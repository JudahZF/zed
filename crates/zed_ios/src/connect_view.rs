...
impl ConnectView {
    pub fn new(_window: &mut Window, cx: &mut Context<Self>) -> Self {
        let hostname_input = cx.new(|cx| {
            TextInput::new("hostname.example.com", cx).with_label("Host")
        });

        let username_input = cx.new(|cx| {
            TextInput::new("username", cx).with_label("Username")
        });
        ...
        let password_input = cx.new(|cx| {
            TextInput::new("", cx).with_label("Password").with_secure(true)
        });
    ...
    fn render_password_prompt(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        match &self.state {
            ConnectionState::WaitingForPassword(prompt) => {
                div()
                    .flex()
                    .flex_col()
                    .gap(px(16.0))
    ...
    fn render_recent_connections(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .when(has_recent, |el| {
                ...
                .children(self.recent_connections.iter().enumerate().map(
                    |(idx, conn)| {
